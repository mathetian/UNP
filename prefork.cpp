...
#ifdef MAP_ANON
	ptr = mmap(NULL, nchildren * sizeof(long), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, -1, 0);
#else
	fd = open("/dev/zero", O_RDWR, 0);
	ptr = mmap(NULL, nchildren * sizeof(long), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, fd, 0);
	close(fd);
#endif
...
void my_lock_init2(char *pathname)
{
	...
	fd = open("/dev/zero", O_RDWR, 0);
	mptr = mmap(NULL, sizeof(pthread_mutex_t), PROT_READ | PROT_WRITE, MAP_ANON | MAP_SHARED, fd, 0);
	close(fd);
